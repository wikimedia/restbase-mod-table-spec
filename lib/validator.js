...
function validateIndexSchema(schema, index) {
    ...
    let sortedIndex = [];
    order.forEach((type) => {
        sortedIndex = sortedIndex.concat(index.filter((idxElem) => {
...
validator.validatePutRequest = function(req, schema) {
    ...
    const missingColumn = Object.keys(req.attributes).find((attrName) => {
